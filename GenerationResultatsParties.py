...
def GenDeBelote():
    ...
    if race==2:
        ...
        race = "IAminimax"
    equipe_2.append([str(joueur), race])
    diffi = affichageOuiNon("Veuillez rentrer le niveau de difficulté des IA (entier; 1: faible; 100 : fort) : ",["1", "5","10","25","50","100"])
    difficulte = int(diffi)
    joueur=[equipe_1[0],equipe_2[0],equipe_1[1],equipe_2[1]]
    
    tirage1= "1"#str(affichageTexteInput("joueur equipe 1 qui tire au sort une carte: ")) ----- est-ce que l'on peut considérer que désigner l'IA qui tire au sort influence le dit tirage ?
